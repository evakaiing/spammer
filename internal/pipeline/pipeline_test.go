...
func TestPipeline(t *testing.T) {
	...
	var ok = true
	var recieved uint32
	freeFlowCmds := []Command{
		Command(func(in, out chan interface{}) {
			out <- 1
...
func TestUsersRace(t *testing.T) {
	...
	service.ResetStat()
	RunPipeline(
		Command(newCatStrings(inputData, 0)),
		Command(SelectUsers),
		Command(newCollectStrings(&testResult)),
	)
...
func TestParallelPiplines(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		RunPipeline(
			Command(newCatStrings(inputData, 150*time.Millisecond)),
			Command(SelectUsers),
	...
	RunPipeline(
		Command(newCatStrings(inputData, 100*time.Millisecond)),
		Command(SelectUsers),
		Command(SelectMessages),
		Command(func(in, out chan interface{}) {
			for range in {
...
func TestTotal(t *testing.T) {
	...
	expectedStat := service.Stat{
		RunGetUser:            uint32(10),
		RunGetMessages:        uint32(5),
		GetMessagesTotalUsers: uint32(9),
		RunHasSpam:            uint32(42),
	}
	assert.Equal(t, expectedStat, service.StatSnapshot(), "количество вызовов функций не совпадает с ожидаемым")
}
